...
# Human-key auto-resolution: subnet can be CIDR like "192.0.2.0/24" → auto-resolved to UUID
...
kea_reservation_{{ res.hostname }}_{{ res.ip_address | replace('.', '_') }}:
  opnsense.item_present:
    ...
    - data:
        # New: subnet can be CIDR "192.0.2.0/24" → auto-resolved to UUID via search
        subnet: {{ res.subnet }}
        ip_address: {{ res.ip_address }}
        hw_address: {{ res.hw_address }}
