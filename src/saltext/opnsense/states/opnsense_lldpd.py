...
"""
Auto-generated OPNsense lldpd state wrappers.
...
def reconfigured(name, controller="service", action="reconfigure"):
    ...
    try:
        __salt__["opnsense.reconfigure"]("lldpd", controller, action)
        ret["result"] = True
        ret["comment"] = f"reconfigured lldpd/{controller}/{action}"
        ret["changes"] = {"reconfigured": f"lldpd/{controller}/{action}"}
