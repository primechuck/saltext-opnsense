...
"""
Auto-generated OPNsense smart state wrappers.
...
def reconfigured(name, controller="service", action="reconfigure"):
    ...
    try:
        __salt__["opnsense.reconfigure"]("smart", controller, action)
        ret["result"] = True
        ret["comment"] = f"reconfigured smart/{controller}/{action}"
        ret["changes"] = {"reconfigured": f"smart/{controller}/{action}"}
