...
"""
Auto-generated OPNsense wol state wrappers.
...
def host_present(name, data=None, match=None, reconfigure=None, search_field=None):
    """
    Ensure host wol present in wol.

    Wraps opnsense.item_present for /api/wol/wol/searchHost
    ...
    """
    return __salt__["opnsense.item_present"](name, "wol", "wol", "host", data, match=match, reconfigure=reconfigure, search_field=search_field)


def host_absent(name, match=None, reconfigure=None, search_field=None):
    """
    Ensure host wol absent in wol.

    Wraps opnsense.item_absent for /api/wol/wol/searchHost
    ...
    :return: State result
    """
    return __salt__["opnsense.item_absent"](name, "wol", "wol", "host", match=match, reconfigure=reconfigure, search_field=search_field)



def reconfigured(name, controller="wol", action="reconfigure"):
    """
    Trigger reconfigure for wol.
    ...
    try:
        __salt__["opnsense.reconfigure"]("wol", controller, action)
        ret["result"] = True
        ret["comment"] = f"reconfigured wol/{controller}/{action}"
        ret["changes"] = {"reconfigured": f"wol/{controller}/{action}"}
