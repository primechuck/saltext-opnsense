...
"""
Auto-generated OPNsense hwprobe state wrappers.
...
def reconfigured(name, controller="service", action="reconfigure"):
    """
    Trigger reconfigure for hwprobe.
    ...
    try:
        __salt__["opnsense.reconfigure"]("hwprobe", controller, action)
        ret["result"] = True
        ret["comment"] = f"reconfigured hwprobe/{controller}/{action}"
        ret["changes"] = {"reconfigured": f"hwprobe/{controller}/{action}"}
