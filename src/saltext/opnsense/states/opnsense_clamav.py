...
"""
Auto-generated OPNsense clamav state wrappers.
...
def url_present(name, data=None, match=None, reconfigure=None, search_field=None):
    """
    Ensure url url present in clamav.

    Wraps opnsense.item_present for /api/clamav/url/searchUrl
    ...
    """
    return __salt__["opnsense.item_present"](name, "clamav", "url", "url", data, match=match, reconfigure=reconfigure, search_field=search_field)


def url_absent(name, match=None, reconfigure=None, search_field=None):
    """
    Ensure url url absent in clamav.

    Wraps opnsense.item_absent for /api/clamav/url/searchUrl
    ...
    :return: State result
    """
    return __salt__["opnsense.item_absent"](name, "clamav", "url", "url", match=match, reconfigure=reconfigure, search_field=search_field)



def reconfigured(name, controller="service", action="reconfigure"):
    """
    Trigger reconfigure for clamav.
    ...
    try:
        __salt__["opnsense.reconfigure"]("clamav", controller, action)
        ret["result"] = True
        ret["comment"] = f"reconfigured clamav/{controller}/{action}"
        ret["changes"] = {"reconfigured": f"clamav/{controller}/{action}"}
