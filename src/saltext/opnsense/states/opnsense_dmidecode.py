...
"""
Auto-generated OPNsense dmidecode state wrappers.
...
def reconfigured(name, controller="service", action="reconfigure"):
    """
    Trigger reconfigure for dmidecode.
    ...
    try:
        __salt__["opnsense.reconfigure"]("dmidecode", controller, action)
        ret["result"] = True
        ret["comment"] = f"reconfigured dmidecode/{controller}/{action}"
        ret["changes"] = {"reconfigured": f"dmidecode/{controller}/{action}"}
