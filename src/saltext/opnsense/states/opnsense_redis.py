...
"""
Auto-generated OPNsense redis state wrappers.
...
def reconfigured(name, controller="service", action="reconfigure"):
    ...
    try:
        __salt__["opnsense.reconfigure"]("redis", controller, action)
        ret["result"] = True
        ret["comment"] = f"reconfigured redis/{controller}/{action}"
        ret["changes"] = {"reconfigured": f"redis/{controller}/{action}"}
