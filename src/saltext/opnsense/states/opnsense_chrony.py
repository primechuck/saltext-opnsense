...
"""
Auto-generated OPNsense chrony state wrappers.
...
def reconfigured(name, controller="service", action="reconfigure"):
    """
    Trigger reconfigure for chrony.
    ...
    try:
        __salt__["opnsense.reconfigure"]("chrony", controller, action)
        ret["result"] = True
        ret["comment"] = f"reconfigured chrony/{controller}/{action}"
        ret["changes"] = {"reconfigured": f"chrony/{controller}/{action}"}
