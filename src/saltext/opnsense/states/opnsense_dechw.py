...
"""
Auto-generated OPNsense dechw state wrappers.
...
def reconfigured(name, controller="info", action="reconfigure"):
    """
    Trigger reconfigure for dechw.
    ...
    try:
        __salt__["opnsense.reconfigure"]("dechw", controller, action)
        ret["result"] = True
        ret["comment"] = f"reconfigured dechw/{controller}/{action}"
        ret["changes"] = {"reconfigured": f"dechw/{controller}/{action}"}
