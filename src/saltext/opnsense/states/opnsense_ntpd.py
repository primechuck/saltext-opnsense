...
"""
Auto-generated OPNsense ntpd state wrappers.
...
def reconfigured(name, controller="service", action="reconfigure"):
    ...
    try:
        __salt__["opnsense.reconfigure"]("ntpd", controller, action)
        ret["result"] = True
        ret["comment"] = f"reconfigured ntpd/{controller}/{action}"
        ret["changes"] = {"reconfigured": f"ntpd/{controller}/{action}"}
