...
"""
Auto-generated OPNsense proxysso state wrappers.
...
def reconfigured(name, controller="service", action="reconfigure"):
    """
    Trigger reconfigure for proxysso.
    ...
    try:
        __salt__["opnsense.reconfigure"]("proxysso", controller, action)
        ret["result"] = True
        ret["comment"] = f"reconfigured proxysso/{controller}/{action}"
        ret["changes"] = {"reconfigured": f"proxysso/{controller}/{action}"}
