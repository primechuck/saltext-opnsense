...
"""
Auto-generated OPNsense zerotier state wrappers.
...
def network_present(name, data=None, match=None, reconfigure=None, search_field=None):
    """
    Ensure network network present in zerotier.

    Wraps opnsense.item_present for /api/zerotier/network/search
    ...
    """
    return __salt__["opnsense.item_present"](name, "zerotier", "network", "network", data, match=match, reconfigure=reconfigure, search_field=search_field)


def network_absent(name, match=None, reconfigure=None, search_field=None):
    """
    Ensure network network absent in zerotier.

    Wraps opnsense.item_absent for /api/zerotier/network/search
    ...
    :return: State result
    """
    return __salt__["opnsense.item_absent"](name, "zerotier", "network", "network", match=match, reconfigure=reconfigure, search_field=search_field)



def reconfigured(name, controller="network", action="reconfigure"):
    """
    Trigger reconfigure for zerotier.
    ...
    try:
        __salt__["opnsense.reconfigure"]("zerotier", controller, action)
        ret["result"] = True
        ret["comment"] = f"reconfigured zerotier/{controller}/{action}"
        ret["changes"] = {"reconfigured": f"zerotier/{controller}/{action}"}
