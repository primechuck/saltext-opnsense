...
"""
Auto-generated OPNsense helloworld state wrappers.
...
def reconfigured(name, controller="service", action="reconfigure"):
    """
    Trigger reconfigure for helloworld.
    ...
    try:
        __salt__["opnsense.reconfigure"]("helloworld", controller, action)
        ret["result"] = True
        ret["comment"] = f"reconfigured helloworld/{controller}/{action}"
        ret["changes"] = {"reconfigured": f"helloworld/{controller}/{action}"}
