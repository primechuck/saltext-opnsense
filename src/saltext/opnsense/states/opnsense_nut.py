...
"""
Auto-generated OPNsense nut state wrappers.
...
def reconfigured(name, controller="diagnostics", action="reconfigure"):
    ...
    try:
        __salt__["opnsense.reconfigure"]("nut", controller, action)
        ret["result"] = True
        ret["comment"] = f"reconfigured nut/{controller}/{action}"
        ret["changes"] = {"reconfigured": f"nut/{controller}/{action}"}
