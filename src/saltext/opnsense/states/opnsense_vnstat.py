...
"""
Auto-generated OPNsense vnstat state wrappers.
...
def reconfigured(name, controller="service", action="reconfigure"):
    """
    Trigger reconfigure for vnstat.
    ...
    try:
        __salt__["opnsense.reconfigure"]("vnstat", controller, action)
        ret["result"] = True
        ret["comment"] = f"reconfigured vnstat/{controller}/{action}"
        ret["changes"] = {"reconfigured": f"vnstat/{controller}/{action}"}
