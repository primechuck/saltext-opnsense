...
"""
Auto-generated OPNsense collectd state wrappers.
...
def reconfigured(name, controller="service", action="reconfigure"):
    """
    Trigger reconfigure for collectd.
    ...
    try:
        __salt__["opnsense.reconfigure"]("collectd", controller, action)
        ret["result"] = True
        ret["comment"] = f"reconfigured collectd/{controller}/{action}"
        ret["changes"] = {"reconfigured": f"collectd/{controller}/{action}"}
