...
"""
Auto-generated OPNsense hostdiscovery state wrappers.
...
def service_present(name, data=None, match=None, reconfigure=None, search_field=None):
    """
    Ensure service service present in hostdiscovery.

    Wraps opnsense.item_present for /api/hostdiscovery/service/search
    ...
    """
    return __salt__["opnsense.item_present"](name, "hostdiscovery", "service", "service", data, match=match, reconfigure=reconfigure, search_field=search_field)


def service_absent(name, match=None, reconfigure=None, search_field=None):
    """
    Ensure service service absent in hostdiscovery.

    Wraps opnsense.item_absent for /api/hostdiscovery/service/search
    ...
    :return: State result
    """
    return __salt__["opnsense.item_absent"](name, "hostdiscovery", "service", "service", match=match, reconfigure=reconfigure, search_field=search_field)



def reconfigured(name, controller="service", action="reconfigure"):
    """
    Trigger reconfigure for hostdiscovery.
    ...
    try:
        __salt__["opnsense.reconfigure"]("hostdiscovery", controller, action)
        ret["result"] = True
        ret["comment"] = f"reconfigured hostdiscovery/{controller}/{action}"
        ret["changes"] = {"reconfigured": f"hostdiscovery/{controller}/{action}"}
