...
"""
Auto-generated OPNsense netsnmp state wrappers.
...
def user_present(name, data=None, match=None, reconfigure=None, search_field=None):
    """
    Ensure user user present in netsnmp.

    Wraps opnsense.item_present for /api/netsnmp/user/searchUser
    ...
    """
    return __salt__["opnsense.item_present"](name, "netsnmp", "user", "user", data, match=match, reconfigure=reconfigure, search_field=search_field)


def user_absent(name, match=None, reconfigure=None, search_field=None):
    """
    Ensure user user absent in netsnmp.

    Wraps opnsense.item_absent for /api/netsnmp/user/searchUser
    ...
    :return: State result
    """
    return __salt__["opnsense.item_absent"](name, "netsnmp", "user", "user", match=match, reconfigure=reconfigure, search_field=search_field)



def reconfigured(name, controller="user", action="reconfigure"):
    """
    Trigger reconfigure for netsnmp.
    ...
    try:
        __salt__["opnsense.reconfigure"]("netsnmp", controller, action)
        ret["result"] = True
        ret["comment"] = f"reconfigured netsnmp/{controller}/{action}"
        ret["changes"] = {"reconfigured": f"netsnmp/{controller}/{action}"}
