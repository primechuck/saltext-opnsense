...
"""
Auto-generated OPNsense apcupsd state wrappers.
...
def reconfigured(name, controller="service", action="reconfigure"):
    """
    Trigger reconfigure for apcupsd.
    ...
    try:
        __salt__["opnsense.reconfigure"]("apcupsd", controller, action)
        ret["result"] = True
        ret["comment"] = f"reconfigured apcupsd/{controller}/{action}"
        ret["changes"] = {"reconfigured": f"apcupsd/{controller}/{action}"}
