...
SPEC_PATHS = [
    pathlib.Path(__file__).parent.parent.parent / "src" / "saltext" / "opnsense" / "utils" / "controllers.json",
...
def test_list_api_modules_returns_76_and_includes_free():
    ...
    free_must_exist = ["caddy", "haproxy", "nginx", "wireguard", "openvpn", "ipsec", "crowdsec", "postfix", "redis", "tor"]
...
def test_generic_exec_import_and_dynamic():
    ...
    for free in ["caddy", "haproxy", "nginx", "wireguard"]:
        assert any(f.startswith(f"{free}_") for f in dynamic), f"generic missing dynamic wrappers for {free}"
...
def test_free_modules_demo_state_exists():
    demo_path = pathlib.Path(__file__).parent.parent.parent / "docs" / "tutorials" / "states" / "free_modules_demo.sls"
    assert demo_path.exists(), "docs/tutorials/states/free_modules_demo.sls must exist to demonstrate free modules import"
    text = demo_path.read_text()
    for needle in ["caddy", "haproxy", "nginx", "76"]:
        assert needle in text

def test_dynamic_only_architecture():
    src_modules = pathlib.Path(__file__).parent.parent.parent / "src" / "saltext" / "opnsense" / "modules"
