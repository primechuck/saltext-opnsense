...
def _cleanup_fake_salt():
    ...
    for name in ["saltext.opnsense.resources.opnsense", "saltext.opnsense.modules.opnsense", "saltext.opnsense.utils.opnsense", "saltext.opnsense.utils.api_spec", "saltext.opnsense.utils.common"]:
...
def test_resource_module_allows_3008(monkeypatch):
    ...
    try:
        ...
        if isinstance(ret, tuple):
            assert "3008" not in ret[1] or "requires" not in ret[1], f"should not reject 3008 for version: {ret}"
        ...
        _cleanup_fake_salt()
        for k in ["requests", "urllib3", "urllib3.util"]:
            if k in sys.modules:
                del sys.modules[k]
...
def test_exec_module_allows_3008(monkeypatch):
    ...
    try:
        ...
        if isinstance(ret, tuple):
            ...
            assert "3008" not in ret[1] or "requires" not in ret[1].lower() or "missing" in ret[1], f"should allow 3008, got {ret}"
        else:
            assert ret is True
    finally:
        _cleanup_fake_salt()
...
def test_packaging_requires_3008():
    ...
    assert '"__proxy__"' not in text, "__proxy__ builtin should be removed after 1.0 Resources migration"
